...
def ustar_data(file_name, longitude, latitude, timezone, data, qc_indicators):
    ...
    with localconverter(robject.default_converter+pandas2ri.converter):
        data_r = robject.conversion.py2rpy(data)
    r_gapfill_indicators = robject.StrVector(gapfill_indicators)
    
    # 调用R函数
    result_data = robject.r['r_co2_flux'](
        r_filename, r_longitude, r_latitude, r_timezone, data_r, r_gapfill_indicators
    )
    
    # 将R结果转回Python
    with localconverter(robject.default_converter + pandas2ri.converter):
        result_data = robject.conversion.rpy2py(result_data)
